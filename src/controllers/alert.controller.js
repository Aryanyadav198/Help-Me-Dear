...
// ─── GET ALL ALERTS (Paginated + Searchable) ──────────────────
const getAllAlerts = asyncHandler(async (req, res) => {
  const page = Math.max(1, parseInt(req.query.page) || 1);
  const search = (req.query.search || '').trim();
  const isSearch = search.length > 0;
  const limit = isSearch ? 20 : 10;
  const skip = (page - 1) * limit;

  // Build optional match stage for text search
  const matchCondition = isSearch
    ? {
      $or: [
  ...
  // Use the extra item only to determine hasMore, never send it to client
  const hasMore = alerts.length > limit;
  const paginated = hasMore ? alerts.slice(0, limit) : alerts;

  const formattedAlerts = paginated.map((alert) => ({
  ...
  return res.status(200).json(
    new ApiResponse(200, 'Alerts fetched', formattedAlerts, { hasMore, page, limit })
  );
});
// const getAllAlerts = asyncHandler(async (req, res) => {
